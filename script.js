...
const DATA = {
  ...
  certificates: [
    ...
    },
    {
      ...
      image: "cert-16thICCCNT-reviewer.png",
      link: "",
},
  ],
...
function renderTraining() {
  ...
  if (!DATA.trainings || DATA.trainings.length === 0) {
    grid.innerHTML = `<p class="cert-empty" style="padding:20px 0">No trainings received listed yet — add them in the <code>DATA.trainings</code> array in script.js.</p>`;
    return;
  }
  ...
  gallery.innerHTML = DATA.certificates.map(cert => `
    <div class="cert-card reveal">
      ...
      <div class="cert-img-wrap">
        ${cert.image
          ? `<img src="${escapeHTML(cert.image)}" alt="${escapeHTML(cert.title)}" class="cert-img"
               onclick="this.closest('.cert-card').querySelector('.cert-lightbox').classList.add('open')" />`
        ...
        ${cert.link ? `<a href="${escapeHTML(cert.link)}" target="_blank" rel="noopener" class="cert-view-btn">View Certificate ↗</a>` : ""}
      </div>
      ...
      <div class="cert-info">
        ...
        ${cert.image ? `<button class="cert-preview-btn" onclick="this.closest('.cert-card').querySelector('.cert-lightbox').classList.add('open')">🔍 View Full Certificate</button>` : ""}
      </div>

      <!-- Lightbox overlay -->
      ${cert.image ? `
        <div class="cert-lightbox" onclick="this.classList.remove('open')">
          <div class="cert-lightbox-inner" onclick="event.stopPropagation()">
            <button class="cert-lightbox-close" onclick="this.closest('.cert-lightbox').classList.remove('open')">✕</button>
            <img src="${escapeHTML(cert.image)}" alt="${escapeHTML(cert.title)}" class="cert-lightbox-img" />
            <div class="cert-lightbox-title">${escapeHTML(cert.title)}</div>
            <div class="cert-lightbox-issuer">${escapeHTML(cert.issuer)} · ${escapeHTML(cert.year)}</div>
          </div>
        </div>
      ` : ""}
    </div>
