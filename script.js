...
const PROFILES = {
  ...
  facebook: {
    url: "https://www.facebook.com/sadmansadikkhan.page/",
  },
  whatsapp: {
    url: "https://wa.me/+8801609655717",
  },
  telegram: {
    url: "https://t.me/Sadmanskhan",
  },
...
const DATA = {
  publications: [
    { title: "Transfer Learning for Fine-Grained Flower Image Classification: A Comparative and Explainable Study", authors: "Mehraj Hossain Mahi, Md. Khaledujjaman, Sadman Sadik Khan, Md. Sams Shahria Sourav, Anzir Rahman Khan, Abdus Sattar", venue: "SPICSCON 2026", publisher: "IEEE", year: "2026", type: "conference", abstract: "Flower image classification is a fine-grained computer vision problem because many flower categories share similar color, shape, petal arrangement, and background patterns. This paper presents a transfer learning-based comparative study using a raw flower image dataset col- lected through physical visits to garden environments. The dataset con- tains 25 flower classes and 9,145 images. The images were divided into training, validation, and test subsets using an approximately 80:10:10 partition, where 7,316 images were used for training, 915 for valida- tion, and 914 for testing. All images were resized and normalized before model training. Five pretrained convolutional neural network backbones, namely MobileNetV2, InceptionV3, VGG16, VGG19, and DenseNet121, were evaluated under a common experimental workflow. Model perfor- mance was assessed using accuracy, loss, macro precision, macro recall, and macro F1-score. DenseNet121 achieved the strongest overall result with 0.9835 training accuracy, 0.9725 validation accuracy, 0.0474 train- ing loss, 0.1141 validation loss, and 0.99 test accuracy. MobileNetV2 also showed competitive performance with 0.9659 validation accuracy and 0.97 test accuracy, indicating that it can serve as a practical alterna- tive when a compact CNN architecture is preferred. The workflow fur- ther includes Grad-CAM and LIME explainability analyses to visually inspect whether the trained models focus on meaningful flower-specific regions during prediction. The findings indicate that pretrained CNNs can provide strong performance for raw garden-collected flower image classification when a consistent preprocessing and evaluation strategy is applied." },
  ...
  projects: [
    ...
    { title: "CNN+GNN Hybrid Architecture", description: "Novel hybrid architecture combining InceptionV3 with GraphSAGE via the Epsilon technique for advanced visual classification tasks.", tags: ["GNN", "CNN", "Hybrid Architecture", "GraphSAGE"], year: "2024" },
  ],
  ...
  education: [
    ...
    { degree: "HSC", field: "Science", university: "Kaliganj Govt. College", year: "2017-2019", gpa: "5.00/5.00", thesis: "" },
    { degree: "SSC", field: "Science", university: "Nalta High School", year: "2015-2017", gpa: "5.00/5.00", thesis: "" },
  ],
  ...
  trainings: [
    // Add trainings you have RECEIVED here, e.g.:
    // { title: "Training Name", organization: "Organizer", year: "2024", description: "What you learned.", icon: "🎓" },
  ],
  ...
  certificates: [
    ...
    },
    {
      ...
      issuer: "Journal of Scientific Research and Reports (JSRR)",
      year: "July 2026",
      description: "Awarded the Certificate of Excellence in Reviewing by the Journal of Scientific Research and Reports (JSRR) in recognition of an outstanding contribution to maintaining the quality of the journal through the peer-review process during the academic year 2026–2027.",
      image: "cert-journal-reviewer-1.jpg",
      link: "",
    },
    {
      id: "cert3",
      title: "Building Impactful Research: Topic Selection, Novelty, and Academic Research Strategy",
      type: "Certificate of Participation",
      issuer: "Division of Research (DoR), Daffodil International University (DIU)",
      year: "31 March 2026",
      description: "Successfully participated in the Seminar organised by the Division of Research (DoR), DIU, held at Daffodil Smart City (DSC). The seminar offered practical tactics on selecting impactful topics, ensuring originality, and developing fruitful research strategies for early-career researchers.",
      image: "cert-impactful-research.jpg",
      link: "",
    },
    {
      id: "cert2",
      title: "Prompt Engineering: Smarter Use of AI for Everyone",
      type: "Certificate of Completion",
      issuer: "Digital Readiness & Intelligent Systems Ltd, Durham, UK",
      year: "24 August 2025",
      description: "Successfully completed the workshop organised by the Digital Readiness & Intelligent Systems Ltd, Durham, UK, held at Online. The workshop focused on prompt engineering, providing practical techniques for effectively interacting with large language models (LLMs) such as ChatGPT (GPT), Claude, Gemini, and other leading LLMs to improve productivity, problem-solving, and content generation.",
      image: "cert-prompt-engineering.jpg",
      link: "",
    },
    {
      id: "cert1",
      title: "Reviewer – 16th IEEE International Conference on Computing, Communication and Networking Technologies (ICCCNT 2025)",
      type: "Certificate of Appreciation",
      issuer: "IEEE ICCCNT 2025",
      year: "6 - 11 July 2025",
      description: "Served as a Reviewer for the 16th IEEE International Conference on Computing, Communication and Networking Technologies (ICCCNT 2025), held at IIT Indore, Madhya Pradesh, India, from 6–11 July 2025. Contributed to the peer-review process by evaluating submitted research papers and supporting the conference's commitment to maintaining high academic and research standards.",
      image: "cert-16th-icccnt-reviewer.jpg",
      link: "",
    },
  ],

  activities: [
    { title: "Trainer — Productivity with AI: Tools Every Teacher Should Know", organization: "Daffodil International University", year: "February 2026", description: "Trained 50+ teachers on AI tools, techniques, guided hands-on AI use for academicians.", category: "training" },
    { title: "Trainer — Advanced ML & Deep Learning Bootcamp", organization: "Daffodil International University", year: "May 2023 – Dec 2023", description: "Trained 100+ students on ML/DL techniques, guided hands-on projects, and promoted practical research skills.", category: "training" },
    ...
    { title: "Organizer, CodeConomics 3.0: Programming Challenge for Non-programmers", organization: "Daffodil International University", year: "Summer 2026", description: "Managed competition platform, logistics, food, and student participation for a national-level event.", category: "event" },
    { title: "Organizer, CodeConomics 2.0: Programming Challenge for Non-programmers", organization: "Daffodil International University", year: "Spring 2026", description: "Managed competition logistics, problem sets, and student participation for a national-level event.", category: "event" },
...
// Global lightbox for book cover and certificates
function openLightbox(imgSrc, title, subtitle) {
  ...
  if (!lb) {
    ...
    lb.innerHTML = `
      <div class="global-lightbox-inner" onclick="event.stopPropagation()">
        <button class="cert-lightbox-close" id="glbClose">✕</button>
        <img id="glbImg" class="global-lightbox-img" src="" alt="" />
        <div id="glbTitle" class="cert-lightbox-title"></div>
        <div id="glbSub" class="cert-lightbox-issuer"></div>
      </div>`;
    lb.addEventListener("click", () => lb.classList.remove("open"));
    document.body.appendChild(lb);
    document.getElementById("glbClose").addEventListener("click", () => lb.classList.remove("open"));
  }
  document.getElementById("glbImg").src = imgSrc;
  document.getElementById("glbImg").alt = title;
  document.getElementById("glbTitle").textContent = title;
  document.getElementById("glbSub").textContent = subtitle || "";
  ...
  container.innerHTML = years.map(year => `
    <div class="year-group reveal">
      ...
      ${byYear[year].map(pub => {
        ...
        return `
        <div class="pub-card ${isBook ? "pub-card-book" : ""}" data-pub-index="${pubIndex}" role="button" tabindex="0" aria-expanded="false">
          <div class="pub-card-row">
            ${pub.coverImage ? `
              <img src="${pub.coverImage}"
                alt="${escapeHTML(pub.title)} cover"
                class="pub-cover-thumb ${isBook ? "pub-cover-thumb-clickable" : ""}"
              />` : ""}
            <div class="pub-card-main">
              <div class="pub-title">${escapeHTML(pub.title)}${isBook ? "" : ` <span class="pub-expand-icon">▾</span>`}</div>
...
function setupPubCardToggle() {
  $$(".pub-card").forEach(card => {
    const isBook = card.classList.contains("pub-card-book");
    const pub = DATA.publications[Number(card.dataset.pubIndex)];

    const action = () => {
      if (isBook) {
        openLightbox(pub.coverImage, pub.title, "ISBN: 978-984-37-0559-4");
      } else {
  ...
  $("#educationList").innerHTML = DATA.education.map(edu => `
    <div class="edu-card reveal">
      ...
      <div class="edu-content">
        ...
        ${edu.gpa  ? `<div class="edu-cgpa">GPA: ${escapeHTML(edu.gpa)}</div>`   : ""}
        ${edu.thesis ? `
...
function renderTraining() {
  ...
  if (!DATA.trainings || DATA.trainings.length === 0) {
    grid.innerHTML = `<p class="cert-empty" style="padding:20px 0">No trainings received listed yet</p>`;
    return;
  }
  grid.innerHTML = DATA.trainings.map(t => `
...
function renderCertificates() {
  ...
  if (!DATA.certificates || DATA.certificates.length === 0) {
    gallery.innerHTML = `<p class="cert-empty">No certificates added yet.</p>`;
    return;
  }

  gallery.innerHTML = DATA.certificates.map(cert => `
    <div class="cert-card reveal">
      <!-- Certificate image -->
      <div class="cert-img-wrap" ${cert.image ? `onclick="openLightbox('${escapeHTML(cert.image)}','${escapeHTML(cert.title)}','${escapeHTML(cert.issuer)} · ${escapeHTML(cert.year)}')"` : ""}>
        ${cert.image
          ? `<img src="${escapeHTML(cert.image)}" alt="${escapeHTML(cert.title)}" class="cert-img" />`
          : `<div class="cert-img-placeholder">
               <span class="cert-placeholder-icon">🏅</span>
               <span class="cert-placeholder-text">Certificate image coming soon</span>
             </div>`
        }
        ${cert.image ? `<div class="cert-img-overlay">🔍 Click to view full size</div>` : ""}
        ${cert.link ? `<a href="${escapeHTML(cert.link)}" target="_blank" rel="noopener" class="cert-view-btn" onclick="event.stopPropagation()">View Certificate ↗</a>` : ""}
      </div>

      <!-- Certificate info -->
      <div class="cert-info">
        <div class="cert-type-badge">${escapeHTML(cert.type)}</div>
        ...
        ${cert.image ? `<button class="cert-preview-btn" onclick="openLightbox('${escapeHTML(cert.image)}','${escapeHTML(cert.title)}','${escapeHTML(cert.issuer)} · ${escapeHTML(cert.year)}')">🔍 View Full Certificate</button>` : ""}
      </div>
    </div>
  `).join("");
...
const CATEGORY_LABELS = { training: "Teaching & Training", leadership: "Leadership", event: "Event Organization", volunteer: "Volunteering", competition: "Competition" };
const CATEGORY_CLASS = { training: "cat-training", leadership: "cat-leadership", event: "cat-event", volunteer: "cat-volunteer", competition: "cat-competition" };
...
function renderProfileLinks() {
  ...
  const ICONS = {
    facebook: `<svg viewBox="0 0 24 24" width="22" height="22" fill="currentColor"><path d="M22 12.06C22 6.5 17.52 2 12 2S2 6.5 2 12.06c0 5.02 3.66 9.18 8.44 9.94v-7.03H7.9v-2.91h2.54V9.85c0-2.51 1.49-3.9 3.77-3.9 1.09 0 2.24.2 2.24.2v2.46h-1.26c-1.24 0-1.63.78-1.63 1.57v1.88h2.78l-.45 2.91h-2.33V22c4.78-.76 8.44-4.92 8.44-9.94z"/></svg>`,
    whatsapp: `<svg viewBox="0 0 24 24" width="22" height="22" fill="currentColor"><path d="M17.47 14.38c-.29-.15-1.7-.84-1.97-.93-.26-.1-.46-.15-.65.14-.2.3-.75.94-.92 1.13-.17.2-.34.22-.63.08-.29-.15-1.22-.45-2.33-1.44-.86-.77-1.44-1.72-1.61-2.01-.17-.3-.02-.46.13-.6.13-.13.29-.34.44-.51.14-.17.19-.3.29-.49.1-.2.05-.37-.02-.51-.08-.15-.65-1.57-.89-2.15-.24-.57-.48-.5-.65-.5-.17 0-.37-.02-.56-.02-.2 0-.51.07-.78.37-.26.3-1.02 1-1.02 2.44s1.05 2.83 1.19 3.03c.15.2 2.06 3.14 5 4.4.7.3 1.24.48 1.67.61.7.22 1.34.19 1.84.12.56-.08 1.7-.7 1.94-1.37.24-.68.24-1.26.17-1.38-.07-.13-.26-.2-.55-.35z"/><path d="M12.02 2C6.5 2 2 6.48 2 12c0 1.87.51 3.63 1.4 5.14L2 22l4.99-1.3A9.95 9.95 0 0 0 12.02 22C17.53 22 22 17.52 22 12S17.53 2 12.02 2zm0 18.14c-1.66 0-3.2-.46-4.52-1.25l-.32-.19-3.03.79.81-2.95-.21-.31A8.12 8.12 0 0 1 3.86 12c0-4.5 3.66-8.14 8.16-8.14 4.5 0 8.14 3.64 8.14 8.14 0 4.5-3.64 8.14-8.14 8.14z"/></svg>`,
    telegram: `<svg viewBox="0 0 24 24" width="22" height="22" fill="currentColor"><path d="M21.94 4.6c.25-1.06-.85-1.9-1.83-1.46L2.4 10.7c-1.03.46-1 1.94.05 2.35l4.44 1.75 1.7 5.46c.24.77 1.22.97 1.76.37l2.47-2.75 4.47 3.32c.86.64 2.1.19 2.34-.86L21.94 4.6zM8.53 13.9l-3.6-1.42 12.15-5.87-9.9 8.9c-.14.13-.24.24-.28.39zm.98 1.68l1.1-3.13 7.06-6.34-6.87 6.86c-.11.11-.19.24-.24.4l-1.05 3.6z"/></svg>`,
  };

  // 3. Contact grid (Contact section) — direct contact details + messaging links
  //    Row 1: Email, Phone. Row 2: Facebook, WhatsApp, Telegram.
  //    (Scholar/ResearchGate/ORCID/etc. already live on the Home page pills.)
  const row1 = [
  ...
  const row2 = [
    { icon: ICONS.facebook, label: "Facebook", href: PROFILES.facebook.url },
    { icon: ICONS.whatsapp, label: "WhatsApp", href: PROFILES.whatsapp.url },
    { icon: ICONS.telegram, label: "Telegram", href: PROFILES.telegram.url },
  ];

  const cardHTML = c => `
    <a href="${c.href}" ${c.href.startsWith("http") ? 'target="_blank" rel="noopener"' : ""} class="contact-card glass-card">
      <div class="contact-icon">${c.icon}</div>
      ${c.value !== undefined
        ? `<div><div class="contact-label">${escapeHTML(c.label)}</div><div class="contact-value">${escapeHTML(c.value)}</div></div>`
        : `<div class="contact-label-solo">${escapeHTML(c.label)}</div>`}
    </a>
  `;

  $("#contactGrid").innerHTML = `
    <div class="contact-row">${row1.map(cardHTML).join("")}</div>
    <div class="contact-row">${row2.map(cardHTML).join("")}</div>
  `;
}
...
// Close global lightbox on Escape key
...
function renderSkills() {
  ...
  if (!container) return;
  container.innerHTML = Object.entries(SKILLS).map(([category, { color, items }]) => `
